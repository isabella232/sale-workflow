...
class SaleOrder(models.Model):
    ...
    def _compute_display_expected_date_ok(self):
        ...
        Displayed if date is set
            - date is set
            - policy is 'when all products are ready' and everything is available
            - policy is 'as soon as possible' and a line is fully available
        """
        for record in self:
            record.display_expected_date_ok = bool(
                record.commitment_date or record.expected_date
            )
