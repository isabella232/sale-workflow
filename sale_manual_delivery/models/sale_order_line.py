...
class SaleOrderLine(models.Model):
    ...
    qty_to_plan = fields.Float(
        string="Quantity to Plan",
        help="There is Pending qty to add to a delivery",
        compute="_compute_qty_to_plan",
        store=True,
        readonly=True,
    )
    ...
    @api.depends("product_uom_qty", "qty_planned")
    def _compute_qty_to_plan(self):
        """ Computes the remaining quantity to plan on sale order lines """
        for line in self:
            line.qty_to_plan = line.product_uom_qty - line.qty_planned
