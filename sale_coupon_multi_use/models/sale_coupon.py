...
class SaleCoupon(models.Model):
    ...
    @api.depends(
        "program_id.discount_fixed_amount",
        "consumption_line_ids.amount"
    )
    def _compute_discount_fixed_amount_delta(self):
        for rec in self:
            amount_total = rec.program_id.discount_fixed_amount
            amount_already_consumed = sum(
                rec.consumption_line_ids.mapped("amount")
            )
            rec.discount_fixed_amount_delta = (
                amount_total - amount_already_consumed
            )
    ...
    def _check_coupon_code(self, order):
        ...
        if self.program_id in order.mapped("coupon_multi_use_ids.program_id"):
            return {
                "error": _(
                    "Multi-Use Coupon is already applied for the same reward"
                )
            }
    ...
    def check_and_update_coupon_state(self):
        ...
        for coupon in self:
            ...
            remaining_delta = (
                float_compare(delta, 0, precision_digits=digits) > 0
            )
