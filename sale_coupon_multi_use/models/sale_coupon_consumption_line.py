...
class SaleCouponConsumptionLine(models.Model):
    ...
    coupon_id = fields.Many2one(
        comodel_name="sale.coupon", required=True, index=True
    )
    sale_order_line_id = fields.Many2one(
        comodel_name="sale.order.line", required=True, ondelete="restrict"
    )
    sale_order_state = fields.Selection(
        related="sale_order_line_id.order_id.state"
    )
    currency_program_id = fields.Many2one(
        related="coupon_id.program_id.currency_id"
    )
    amount = fields.Float(
